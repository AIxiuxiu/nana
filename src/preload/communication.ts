 ...
 */
import { clipboard, contextBridge, Dialog, ipcRenderer, IpcRenderer, shell, webFrame } from 'electron';
import * as fs from 'fs';

console.log('start electron preload!');
...
try {
  ...
  contextBridge.exposeInMainWorld('setZoomFactor', setZoomFactor);
} catch {
  (window as any).platform = process.platform;
  (window as any).fs = fs;
  (window as any).electron = api;
  (window as any).setZoomFactor = setZoomFactor;
}
