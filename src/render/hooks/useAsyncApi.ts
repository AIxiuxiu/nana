...
import { InjectTableKey } from '@/types/symbols';
...
export function useAsyncTableApi<R, Args extends any = any>(apiFun: (args: Args) => Promise<QjResponse<R>>, options?: AsyncTableApiOptions<R>) {
  ...
  const asyncApi = useAsyncApi(apiFun, { initialData: [], ...options });
  ...
  provide(InjectTableKey(id), { pageSize: pageSizeRef, currentPage, onCurrentChange, onSizeChange, totalCount: asyncApi.total, refresh: asyncApi.refresh });
